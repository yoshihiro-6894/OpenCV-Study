...
def mean_filter(image, boundary='reflect'):
    ...
    kernel_5=np.array([[0.354,0.447,0.5,0.447,0.354],[0.447,0.707,1,0.707,0.447],[0.5,1,0,1,0.5],[0.447,0.707,1,0.707,0.447],[0.354,0.447,0.5,0.447,0.354]])
    ...
    for i in range((image.shape[0])):
      for j in range((image.shape[1])):
        if img_binary[i][j] == 1:
          img_cp[i][j]=W_mean(strided_image_5[i][j],(kernel_5_1*(np.ones(binary_5[i][j].shape)-binary_5[i][j])))
