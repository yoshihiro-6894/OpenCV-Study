...
def W_mean(img,kernel):
  ...
  if a.shape[0]<1:
    return img[math.floor(img.shape[0]/2)]
  return np.mean(a)

def mean_filter(image, boundary='reflect'):
    ...
    kernel_5=np.array([[1,2,3,2,1],[2,0,0,0,2],[3,0,0,0,3],[2,0,0,0,2],[1,2,3,2,1]])
    kernel_5_1=kernel_5.reshape(kernel_5.shape[0]*kernel_5.shape[1])
    kernel_7=np.array([[1,2,3,4,3,2,1],[2,0,0,0,0,0,2],[3,0,0,0,0,0,3],[4,0,0,0,0,0,4],[3,0,0,0,0,0,3],[2,0,0,0,0,0,2],[1,2,3,4,3,2,1]])
    kernel_7_1=kernel_7.reshape(kernel_7.shape[0]*kernel_7.shape[1])
    kernel9=np.array([[1,2,3,4,5,4,3,2,1],[2,0,0,0,0,0,0,0,2],[3,0,0,0,0,0,0,0,3],[4,0,0,0,0,0,0,0,4],[5,0,0,0,0,0,0,0,5],[4,0,0,0,0,0,0,0,4],[3,0,0,0,0,0,0,0,3],[2,0,0,0,0,0,0,0,2],[1,2,3,4,5,4,3,2,1]])
    kernel9_1=kernel9.reshape(kernel9.shape[0]*kernel9.shape[1])
    ...
    strided_image_9 = pad_stride(image,kernel9,boundary)
    ...
    print("ノイズ検出")
    t=time.time()
    img_binary=np.apply_along_axis(W,2,strided_image_11)
    tt=time.time()
    print("時間")
    print(tt-t)
    ...
    binary_9=pad_stride(img_binary,kernel9,boundary)
    ...
    for i in range((image.shape[0])):
      for j in range((image.shape[1])):
        if img_binary[i][j] == 1:
          if np.count_nonzero(binary_3[i][j]==0) < 4:
            #3*3
            img_cp[i][j] = W_median(strided_image_3[i][j], (kernel_3_1*(np.ones(binary_3[i][j].shape)-binary_3[i][j]))) 
          else:
            #5*5
            img_cp[i][j] = W_mean(strided_image_3[i][j], (kernel_3_1*(np.ones(binary_3[i][j].shape)-binary_3[i][j])))
    '''
    cv2.imshow("pass1",img_cp)
    strided_image_11 = pad_stride(img_cp,kernel_11,boundary)
    img_binary=np.apply_along_axis(W,2,strided_image_11)
    for i in range((image.shape[0])):
      for j in range((image.shape[1])):
        if img_binary[i][j] == 1:
